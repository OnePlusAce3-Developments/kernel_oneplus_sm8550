...
struct timer_list bright_panel_timer;
...
#if IS_ENABLED(CONFIG_DRM_PANEL_NOTIFY)
static int oplus_theia_bright_check_panel_dt(void) {
	int i;
	int count;
	...
	if (!np) {
		BRIGHT_DEBUG_PRINTK("Device tree info missing.\n");
		return -EINVAL;
	} else {
		BRIGHT_DEBUG_PRINTK("Device tree info found.\n");
	}

	count = of_count_phandle_with_args(np, "panel", NULL);
	BRIGHT_DEBUG_PRINTK("Found  panel nums = %d.\n", count);
	if (count <= 0) {
		return 0;
	}
	...
	for (i = 0; i < count; i++) {
		...
		if (!IS_ERR(panel)) {
			...
			BRIGHT_DEBUG_PRINTK("Found active panel.\n");
			return 0;
		}
	}
...
static int bright_panel_event_register() {
	void *cookie = NULL;
	void *data = NULL;

	cookie = panel_event_notifier_register(PANEL_EVENT_NOTIFICATION_PRIMARY,
		PANEL_EVENT_NOTIFIER_CLIENT_PRIMARY_THEIA_BRIGHT, g_bright_data.active_panel,
		&bright_fb_notifier_callback, data);
	if (!cookie) {
		BRIGHT_DEBUG_PRINTK("Unable to register theia_panel_notifier.\n");
		return -1;
	} else {
		BRIGHT_DEBUG_PRINTK("Success register theia_panel_notifier.\n");
		return 0;
	}
	return 0;
}

static void bright_panel_dt_timer_func(struct timer_list *t) {
	BRIGHT_DEBUG_PRINTK("bright_panel_dt_timer_func is called\n");

	if (g_bright_data.active_panel == NULL) {
		BRIGHT_DEBUG_PRINTK("Retry oplus_theia_bright_check_panel_dt\n");
		oplus_theia_bright_check_panel_dt();
		mod_timer(&bright_panel_timer, jiffies + msecs_to_jiffies(2000));
	} else {
		BRIGHT_DEBUG_PRINTK("bright_panel_event_register called\n");
		bright_panel_event_register();
		del_timer(&bright_panel_timer);
	}
}
...
#if IS_ENABLED (CONFIG_DRM_PANEL_NOTIFY)
	oplus_theia_bright_check_panel_dt();
	timer_setup(&bright_panel_timer, (bright_panel_dt_timer_func), TIMER_DEFERRABLE);
	mod_timer(&bright_panel_timer, jiffies + msecs_to_jiffies(2000));
#elif IS_ENABLED (CONFIG_DRM_MSM)
