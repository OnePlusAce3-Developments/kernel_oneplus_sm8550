...
struct timer_list black_panel_timer;
...
#if IS_ENABLED(CONFIG_DRM_PANEL_NOTIFY)
static int oplus_theia_black_check_panel_dt(void) {
	int i;
	int count;
...
static int black_panel_event_register() {
	void *cookie = NULL;
	void *data = NULL;

	cookie = panel_event_notifier_register(PANEL_EVENT_NOTIFICATION_PRIMARY,
		PANEL_EVENT_NOTIFIER_CLIENT_PRIMARY_THEIA_BLACK, g_black_data.active_panel,
		&black_fb_notifier_callback, data);
	if (!cookie) {
		BLACK_DEBUG_PRINTK("Unable to register black_theia_panel_notifier.\n");
		return -1;
	} else {
		BLACK_DEBUG_PRINTK("Success register black_theia_panel_notifier.\n");
		return 0;
	}
	return 0;
}

static void black_panel_dt_timer_func(struct timer_list *t) {
	BLACK_DEBUG_PRINTK("black_panel_dt_timer_func is called\n");

	if (g_black_data.active_panel == NULL) {
		BLACK_DEBUG_PRINTK("Retry oplus_theia_black_check_panel_dt\n");
		oplus_theia_black_check_panel_dt();
		mod_timer(&black_panel_timer, jiffies + msecs_to_jiffies(2000));
	} else {
		BLACK_DEBUG_PRINTK("black_panel_event_register called\n");
		black_panel_event_register();
		del_timer(&black_panel_timer);
	}
}
...
#if IS_ENABLED (CONFIG_DRM_PANEL_NOTIFY)
	oplus_theia_black_check_panel_dt();
	timer_setup(&black_panel_timer, (black_panel_dt_timer_func), TIMER_DEFERRABLE);
	mod_timer(&black_panel_timer, jiffies + msecs_to_jiffies(2000));
#elif IS_ENABLED (CONFIG_DRM_MSM)
